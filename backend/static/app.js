...
function statusMode(mode,text){dot.className='va-dot va-dot-'+mode;status.textContent=text;}
...
async function start(){
  ...
  function checkSilence(){
    ...
    for(let i=0;i<data.length;i++){
      const v=(data[i]-128)/128; rms+=v*v;
    }
    rms=Math.sqrt(rms);
    if(rms<SILENCE_THRESHOLD){
      if(!silenceTimer){
        silenceTimer=setTimeout(()=>{stop();},SILENCE_DURATION);
      }
    }else{
